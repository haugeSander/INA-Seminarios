...
public class Dispositivo_APIMQTT implements MqttCallback {
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		String action = payload.getString("accion").trim();

		String[] topicNiveles = topic.split("/");
		if (topicNiveles[topicNiveles.length - 1].equalsIgnoreCase("comandos")) {
		...
		if (Arrays.stream(topicNiveles).anyMatch("funcion"::equals)) {
			...
			switch (action.toLowerCase()) {
				...
				case "parpadear":
					...
					break;
				default:
					MySimpleLogger.warn(this.loggerId, "Acción '" + action + "' no reconocida para función.");
			}
		} else {
			switch (action.toLowerCase()) {
				case "habilitar":
				...
				case "deshabilitar":
					...
					break;
				default:
					MySimpleLogger.warn(this.loggerId, "Acción '" + action + "' no reconocida para dispositivo.");
			}
		}
